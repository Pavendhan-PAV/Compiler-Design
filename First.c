...
void First(char c, int hop, int flag)
{
	...
	if (c != '#')
	{
		for (i = 0; i < prodCount; i++)
		{
			...
			{
				if (!strchr(first, c) && !strchr(unique_nt, c)) // if not already in set & not a Non-Terminal
				{
					first[f_count] = c;
					f_count++;
				}
	...
	{
		if (!strchr(first, c) && isStartingVariable != 1)
		{
			first[f_count] = c;
			f_count++;
		}
...
int main()
{
	int i, j = 0, flag = 0;
	f_count = 0;
	printf("\nCONTRAINTS: \n\t1: If there are multiple entries in same production, split them and enter.\n\t\tS->A|B == S->A and S->B\n\n\t2: Productions of form S->SAbc where F(S)=F(S) are not allowed.\n\n\t3: Epsilon is '#'\n");
	...
	{
		if (k == 0)
			isStartingVariable = 1;
		else
			isStartingVariable = 0;
